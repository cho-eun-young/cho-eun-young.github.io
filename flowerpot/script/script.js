$(function(){
    var $section = $(".section");
    var $slider = $('#main_wrap'),
        $firstSlide = $slider.find('.con').first() // 첫번째 슬라이드
        .stop().animate({'opacity':1},200); // 첫번째 슬라이드만 보이게 하기
  
    function PrevSlide(){ // 이전버튼 함수
      stopSlide();startSlide(); //타이머 초기화
      var $lastSlide = $slider.find('.con').last() //마지막 슬라이드
      .prependTo($slider); //마지막 슬라이드를 맨 앞으로 보내기  
      $secondSlide = $slider.find('.con').eq(1)//두 번째 슬라이드 구하기
      .stop(true).animate({'opacity':0},500); //밀려난 두 번째 슬라이드는 fadeOut 시키고
      $firstSlide = $slider.find('.con').first() //맨 처음 슬라이드 다시 구하기
      .stop(true).animate({'opacity':1},500);//새로 들어온 첫 번째 슬라이드는 fadeIn 시키기
    }
    
    function NextSlide(){ // 다음 버튼 함수
      stopSlide();startSlide(); //타이머 초기화
      $firstSlide = $slider.find('.con').first() // 첫 번째 슬라이드
      .appendTo($slider); // 맨 마지막으로 보내기
      var $lastSlide = $slider.find('.con').last() // 맨 마지막으로 보낸 슬라이드
      .stop(true).animate({'opacity':0},500); // fadeOut시키기
      $firstSlide = $slider.find('.con').first()// 맨 처음 슬라이드
   ...
   $section.hover(function(){ //마우스 오버시 슬라이드 멈춤
      stopSlide();
    }, function (){
      startSlide();
    });    
    ...
    var $btn = $(".btn")
    $btn.hover(function(){ //마우스 오버시 슬라이드 멈춤
      stopSlide();
    }, function (){
      startSlide();
    });    
